import { Open20Regular, TextGrammarWand20Regular, RecordStopFilled, ArrowSyncRegular} from "@fluentui/react-icons";
  ...
  return {
    ...
    draftSeed: modified
      ? {
          ...
          isPrivate: essay.localIsPrivate ?? essay.isPrivate === true,
          themeId:
...
function AppContent() {
  ...
  const analysisRepository = useMemo(createAnalysisRepository, []);
  const llmClient = useMemo(createOpenAiCompatibleClient, []);
  ...
  const activeDocumentKey = activeDocument
    ? getDocumentKey(activeDocument)
    : "loading";
  const draft = useDraftController({
    baselineContent:
      activeDocument?.kind === "published" ? activeDocument.content : "",
    baselineIsPrivate:
      activeDocument?.kind === "published" ? activeDocument.isPrivate : false,
    baselineThemeId:
      activeDocument?.kind === "published" ? activeDocument.themeId : null,
  ...
  const notifyAnalysisSaveError = useCallback(() => {
    toast({
      title: "无法保存分析结果",
      description: "本次结果仍会在当前运行期间保留",
      variant: "destructive",
    });
  }, [toast]);
  ...
  const handleEditorReady = useCallback(
    (documentKey: string, editorMetrics?: MarkdownEditorReadyMetrics) => {
      const pending = pendingSwitchMeasureRef.current;
      if (
        !import.meta.env.DEV ||
        import.meta.env.MODE === "test" ||
  ...
  const resolveThemeId = useCallback(
    (themeSlug: string | null) =>
      themeSlug === null
        ? null
        : (themes.themes.find((theme) => theme.slug === themeSlug)?.id ?? null),
    [themes.themes],
  );
  ...
  const handlePublishSuccess = useCallback(
    async (id: string) => {
      ...
      setLocalDrafts((current) =>
        current.filter((draft) => draft.localId !== publishedDraft.localId),
      );
      setActiveDocument((current) =>
        current?.kind === "draft" && current.localId === publishedDraft.localId
          ? {
              kind: "published",
              id,
      ...
      void analysisRepository
        .move(
          getLocalDraftDocumentKey(publishedDraft.localId),
          `essay:${id}`,
          createContentFingerprint(publishedDraft.content),
        )
        .catch(notifyAnalysisSaveError);

      try {
        await draftRepository.removeLocalDraft(publishedDraft.localId);
      } catch {
        toast({
  ...
  useEffect(() => {
    if (activeDocument?.kind !== "draft" || !draft.ready || !localDraftsReady) {
      return;
    }
    ...
    activeDocument?.kind,
    activeDocument?.kind === "draft" ? activeDocument.localId : null,
    draft.content,
    draft.isPrivate,
    draft.ready,
    draft.themeId,
  ...
  useEffect(() => {
    ...
    const flushBeforeClose = async () => {
      analysis.cancel();
      const [draftSaved, analysisSaved, settingsSaved] = await Promise.all([
        draft.flushAll(),
        analysis.flush(),
        settings.flushAllSettings(),
      ]);
      return draftSaved && analysisSaved && settingsSaved;
    };
    void tauriDesktopAdapter
      .interceptClose(flushBeforeClose)
      .then((nextUnlisten) => {
        if (cancelled) {
          nextUnlisten();
        } else {
  ...
  const returnToEditor = async () => {
    const saved = await settings.flushAllSettings();
    if (!saved) {
      return;
    }
  ...
  const startAnalysis = async () => {
    const result = await analysis.start();
    if (result === "not-configured") {
      openSettings();
      toast({
        title: "请先正确配置大模型",
        description: "填写 Base URL、API Key 和模型后，请先测试连接",
        variant: "destructive",
      });
  ...
  const deleteDocument = async () => {
    ...
    try {
      if (target.kind === "draft") {
        ...
        void analysisRepository
          .clear(getDocumentKey(target))
          .catch(notifyAnalysisSaveError);
        setLocalDrafts(remainingDrafts);
        await activateFirstAvailableDocument(remainingDrafts, library.entries);
        return true;
      }
      ...
      await essayLibraryClient.remove(target.id, settings.accessToken);
  ...
  const createDraft = async () => {
    ...
    const emptyDraft = localDraftsRef.current.find(
      (entry) =>
        !entry.content.trim() && !entry.isPrivate && entry.themeId === null,
    );
    if (emptyDraft) {
    ...
    try {
      const nextDraft = await draftRepository.createLocalDraft();
      setLocalDrafts((current) => sortDrafts([nextDraft, ...current]));
      setActiveDocument(activateDraft(nextDraft));
      showEditor();
    } catch {
      notifyDraftError();
  ...
  const activateDocument = (nextDocument: ActiveDocument) => {
    ...
    const saveTask = activeDocument ? draft.flush() : Promise.resolve(true);
  ...
  const exportDocx = async () => {
    ...
    try {
      const { createMarkdownDocx } =
        await import("@/features/editor/markdown-docx");
  ...
  const activeEssayEntry =
    activeDocument?.kind === "published"
      ? library.entries.find((entry) => entry.id === activeDocument.id)
      : undefined;
  const documentStatus =
    activeDocument?.kind === "published"
      ? getPublishedDocumentStatus({
          ...
          hasKnownLocalChanges: activeEssayEntry?.localContent !== undefined,
        })
      : "draft";
  const editorStatusLabel =
    documentStatus === "draft"
      ...
      : documentStatus === "published"
        ? "已发布"
        : "已发布（在本地有更改）";
  const actionLabel = documentStatus === "modified" ? "更新文章" : "发布文章";
  ...
  return (
    <AppShell
      ...
      editorToolbarActions={
        <>
          {analysis.running ? (
            <div className="rounded-full flex justify-between items-center  bg-gray-200 dark:bg-gray-800 py-1 px-2 gap-3">
              <button
                type="button"
                className="editor-analysis-button is-running  flex justify-start items-center gap-1"
                aria-label="正在分析"
                title={
                  analysis.progress.total
                    ? `正在分析 ${analysis.progress.completed}/${analysis.progress.total}`
                    : "正在分析"
                }
                disabled
              >
                <ArrowSyncRegular className="animate-spin" aria-hidden="true" />
                <span className="text-[11px]">
                  {analysis.progress.total
                    ? `分析 ${analysis.progress.completed}/${analysis.progress.total}`
                    : "分析中"}
                </span>
              </button>
              <button
                type="button"
                className="editor-analysis-cancel-button"
                aria-label="取消分析"
                title="取消分析"
                onClick={analysis.cancel}
              >
                <RecordStopFilled aria-hidden="true" />
              </button>
            </div>
          ) : (
            <button
              type="button"
              className="editor-analysis-button"
              aria-label="分析文章"
              title="分析"
              disabled={Boolean(
                !activeDocument ||
            ...
            >
              <TextGrammarWand20Regular aria-hidden="true" />
            </button>
          )}
          <ThemeSelector
            disabled={Boolean(
          ...
          <DeleteDocumentButton
            ...
            disabled={Boolean(
              ...
              updating ||
              analysis.running ||
              (activeDocument.kind === "published" && !settings.accessToken),
            )}
      ...
      sidebarContent={
        <SidebarEssayList
          ...
          onRetry={activity.error ? activity.refresh : library.retry}
    ...
    >
      <EditorPage
        ...
        content={draft.content}
        disabled={
          publishing.loading || updating || deleting || analysis.running
        }
        documentKey={activeDocumentKey}
        isPrivate={draft.isPrivate}
        loading={publishing.loading || updating}
        onContentChange={handleContentChange}
        onAnalysisIssuesChange={analysis.replaceIssues}
        ...
        ready={Boolean(activeDocument && draft.ready && localDraftsReady)}
      />
      <div
      ...
      >
        <SettingsPage
          ...
          llmSaveStatus={settings.llmSaveStatus}
          ...
          onDiscoverModels={() => void settings.discoverModels()}
          onLlmApiKeyChange={settings.changeLlmApiKey}
          onLlmBaseUrlChange={settings.changeLlmBaseUrl}
          onLlmBlur={() => {
            void settings.flushLlmSettings().then((saved) => {
              if (
                saved &&
                settings.llmSettings.baseUrl.trim() &&
                settings.llmSettings.apiKey.trim()
              ) {
                void settings.discoverModels();
              }
            });
          }}
