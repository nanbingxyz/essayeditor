...
interface SettingsPageProps {
  ...
  onLlmReasoningEnabledChange: (enabled: boolean) => void;
  onTestLlm: () => void;
    ...
    <div className="settings-page flex flex-col">
      ...
      <section className="settings-section" aria-labelledby="api-key-heading">
        <div className="settings-section-copy">
          ...
          <div className="api-key-description">
            <p>在 Essay 个人设置的“API 设置”中启用 API 并获取 API Key。</p>
            <Popover.Root>
              <Popover.Trigger
      ...
      >
        <div className="settings-section-copy">
          ...
          <p>配置兼容 OpenAI API 的服务。Base URL 通常以 /v1 结尾。</p>
        </div>
        ...
        <div className="llm-settings-fields">
          ...
          <div className="api-key-field">
            ...
            <Button
              ...
              disabled={disabled}
              aria-label={
                showLlmApiKey ? "隐藏大模型 API Key" : "显示大模型 API Key"
              }
              onClick={() => setShowLlmApiKey((visible) => !visible)}
            >
              {showLlmApiKey ? <EyeClosedIcon /> : <EyeOpenIcon />}
            </Button>
          </div>
          ...
          <div className="llm-model-heading">
            ...
            >
              <ReloadIcon
                className={
                  modelStatus === "loading" ? "animate-spin" : undefined
                }
              />
              获取模型
            </Button>
          ...
          {modelStatus === "error" && (
            <p className="llm-inline-status is-error">{modelError}</p>
          )}
          <label className="llm-reasoning-row" htmlFor="llm-reasoning">
            <span className="llm-reasoning-copy">
              <span>开启推理</span>
              <small>开启后分析耗时更长，但质量通常会提高。</small>
            </span>
            <input
              id="llm-reasoning"
              className="llm-reasoning-switch"
              type="checkbox"
              role="switch"
              checked={llm.reasoningEnabled}
              disabled={disabled}
              aria-label="开启推理"
              onChange={(event) =>
                onLlmReasoningEnabledChange(event.target.checked)
              }
            />
          </label>
      ...
      <div className="mt-6">
        <button
          className="text-muted-foreground text-sm font-serif font-medium"
          onClick={() =>
            tauriDesktopAdapter.openExternal("https://www.essay.ink")
          }
        >
          Essay
        </button>
