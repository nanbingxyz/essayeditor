...
import { analyzeWriting, createContentFingerprint } from "./writing-analyzer";

type AnalysisStatus = "idle" | "loading" | "running" | "completed" | "error";
...
export type StartAnalysisResult =
  "started" | "not-configured" | "empty" | "busy";
...
export function useAnalysisController({
    ...
    async (
      ...
      try {
        await repository.save(
          createSnapshot(targetDocumentKey, targetContent, targetIssues),
        );
        return true;
      } catch {
  ...
  const replaceIssues = useCallback(
    (nextIssues: AnalysisIssue[], nextContent?: string) => {
      ...
      saveTimerRef.current = setTimeout(() => {
        ...
        if (pending) {
          void persist(pending.documentKey, pending.content, pending.issues);
        }
      }, 500);
    },
    [persist],
  ...
  const flush = useCallback(async () => {
    ...
    pendingSaveRef.current = undefined;
    return pending
      ? persist(pending.documentKey, pending.content, pending.issues)
      : true;
  }, [persist]);
  ...
  const start = useCallback(async (): Promise<StartAnalysisResult> => {
    ...
    void analyzeWriting({
      ...
      .then(async (nextIssues) => {
        ...
        await persist(targetDocumentKey, targetContent, nextIssues);
        issuesRef.current = nextIssues;
        setIssues(nextIssues);
        setStatus("completed");
      ...
      .catch((error) => {
        ...
        setStatus("error");
        onErrorRef.current(
          error instanceof Error ? error.message : "写作分析失败，请稍后重试",
        );
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    void repository
      ...
      .then((snapshot) => {
        ...
        const loaded =
          snapshot?.checklistVersion === WRITING_CHECKLIST_VERSION
